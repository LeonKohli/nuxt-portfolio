    ...
    <section id="tech-stack" class="flex flex-col justify-center min-h-screen px-0 py-24 overflow-hidden sm:px-6 lg:px-8">
        <div class="w-10/12 md:w-8/12 mx-auto max-w-[110rem] px-4">
            ...
            <div class="flex flex-col items-center mb-16 text-center md:items-start md:text-left">
                ...
                <p class="mt-4 text-lg opacity-0 text-white/60 animate-fade-in" style="animation-delay: 600ms;">
            ...
            <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
                <a v-for="(tech, index) in techStack?.technologies" :key="tech.name" :href="tech.url" target="_blank"
                    rel="noopener noreferrer" class="group relative p-3 transition-all duration-300 rounded-xl border border-white/10 
                 hover:border-white/20 bg-white/[0.02] hover:bg-white/[0.04] hover:-translate-y-1
                 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 opacity-0 animate-slide-up"
                    ...
                    <div class="relative z-10 flex items-center gap-3">
                        <div class="flex items-center justify-center w-10 h-10 rounded-lg shrink-0 transition-all duration-300
                     shadow-[0_0_0_1px_rgba(255,255,255,0.1)] group-hover:shadow-[0_0_0_1.5px_rgba(255,255,255,0.2)]
                     group-hover:scale-110" :style="{ backgroundColor: `${tech.color}15` }">
                        ...
                        <div class="min-w-0">
                            <h4
                                class="text-sm font-medium truncate transition-colors duration-300 text-white/90 group-hover:text-white">
                                {{ tech.name }}
                            </h4>
                            <p
                                class="text-xs truncate transition-colors duration-300 text-white/60 group-hover:text-white/80">
                                {{ tech.category }}
                            </p>
                        </div>
...
if (!techStack.value) {
    throw createError('Failed to load tech stack data')
}
</script>
